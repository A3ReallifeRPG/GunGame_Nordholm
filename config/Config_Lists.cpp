class base_list_weapon {
	kills = 2;
	mag = "";
	mag_count = 5;
	scope = "ace_optic_arco_2d";
};

class Lists {

	class rl_weapons { // 42 Kills
		class RL_crossbow_black : base_list_weapon {};
		class RL_W_Desert_Eagle_Blacksky : base_list_weapon {
			kills = 2;
			scope = "optic_mrd_black";
		};
		class RL_W_glock18_war : base_list_weapon {
			kills = 4;
			scope = "optic_mrd_black";
			mag = "RL_M_30Rnd_9x19_Mag";
			mag_count = 10;
		};
		class RL_MP7_Rifle_Black_40Rnd : base_list_weapon {};
		class RL_W_ar15_slr : base_list_weapon {};
		class RL_W_ar15_skeletonized : base_list_weapon {};
		class hlc_rifle_RU556 : base_list_weapon {};
		class hlc_rifle_saiga12k : base_list_weapon {
			kills = 4;
			scope = "hlc_optic_kobra";
			mag = "hlc_10rnd_12g_slug_S12";
		};
		class RL_W_asr14 : base_list_weapon {};
		class RL_W_MCR_Carbine_Damascus : base_list_weapon {};
		class hlc_rifle_FAL5061Rail : base_list_weapon {};
		class RL_W_famas_g2 : base_list_weapon {};
		class LMG_03_F : base_list_weapon {
			kills = 3;
			scope = "optic_aco_grn";
		};
	  class RL_W_SCAR_H_GOLD : base_list_weapon {};
		class RL_W_f2000 : base_list_weapon {};
		class RL_W_p90 : base_list_weapon {
			kills = 3;
			scope = "optic_hamr";
			mag_count = 6;
		};
		class hlc_wp_XM177E2 : base_list_weapon {};
		class RL_W_Sword_Mk18 : base_list_weapon {
			kills = 3;
			scope = "hlc_optic_leupoldm3a";
			mag_count = 6;
		};
		class hlc_rifle_m14sopmod : base_list_weapon {};
		class RL_W_MDR_762 : base_list_weapon {
			kills = 3;
			mag_count = 6;
			scope = "optic_sos";
		};
		class RL_Glock_17 : base_list_weapon {};
		class RL_W_Desert_Eagle_Gold : base_list_weapon {
			kills = 3;
		};
	};

	class new_rl_weapons { // 42 Kills
		class RL_W_Desert_Eagle_Blacksky : base_list_weapon {
			kills = 2;
			scope = "optic_mrd_black";
		};
		class RL_W_glock18_war : base_list_weapon {
			kills = 4;
			scope = "optic_mrd_black";
			mag = "RL_M_30Rnd_9x19_Mag";
			mag_count = 10;
		};

		class RL_W_ar15_slr : base_list_weapon {};
		class RL_W_ar15_skeletonized : base_list_weapon {};
		class RL_W_asr14 : base_list_weapon {};
		class RL_W_MCR_Carbine_Damascus : base_list_weapon {};
		class RL_W_MDR_556 : base_list_weapon {};
		class RL_W_famas_g2 : base_list_weapon {};
	  class RL_W_SCAR_H_GOLD : base_list_weapon {};
		class RL_W_f2000 : base_list_weapon {};
		class RL_W_p90 : base_list_weapon {
			kills = 3;
			scope = "optic_hamr";
			mag_count = 6;
		};
		class RL_W_MDR_RAINBOW_556 : base_list_weapon {};
		class RL_W_MCR_Carbine : base_list_weapon {};
		class RL_W_Sword_Mk18 : base_list_weapon {
			kills = 3;
			scope = "hlc_optic_leupoldm3a";
			mag_count = 6;
		};
		class RL_W_SCAR_H_BLACK : base_list_weapon {};
		class RL_W_MDR_762 : base_list_weapon {
			kills = 3;
			mag_count = 6;
			scope = "optic_sos";
		};
		class RL_W_Desert_Eagle_Gold : base_list_weapon {
			kills = 3;
		};
	};

	class copShop {  //23 kills total
		class hlc_Pistol_P228 : base_list_weapon {
			kills = 1;
			scope = "hlc_optic228_docter_cadex";
		};
		class hlc_mp510_tac : base_list_weapon {};
		class hlc_rifle_CQBR : base_list_weapon {};
		class hlc_rifle_auga3_bl : base_list_weapon {
			kills = 1;
		};
		class hlc_rifle_G36KV : base_list_weapon {
			kills = 1;
		};
		class hlc_rifle_SG551LB_TAC : base_list_weapon {};
		class hlc_rifle_M4 : base_list_weapon {};
		class hlc_rifle_SG550Sniper_RIS : base_list_weapon {
			scope = "ace_optic_lrps_pip";
		};
		class hlc_rifle_416D10 : base_list_weapon {
			kills = 1;
		};
		class hlc_rifle_bcmjack : base_list_weapon {};
		class hlc_rifle_M14dmr_Rail : base_list_weapon {};
		class hlc_rifle_M27IAR : base_list_weapon {};
		class hlc_rifle_falosw : base_list_weapon {};
		class hlc_rifle_ACR_MID_black : base_list_weapon {};
		class hlc_rifle_g3a3ris : base_list_weapon {};
		class hlc_rifle_416D20 : base_list_weapon {};
		class hlc_rifle_m14sopmod : base_list_weapon {
			mag = "hlc_50Rnd_762x51_TDIM_M14";
			scope = "optic_ams";
		};
		class hlc_rifle_SAMR : base_list_weapon {};
		class RL_MP7_Rifle_Green_40Rnd : base_list_weapon {
			kills = 3;
			scope = "optic_aco_grn";
		};
		class hlc_rifle_RU556 : base_list_weapon {};
		class hlc_rifle_m4m203 : base_list_weapon {
			kills = 3;
			mag = "1Rnd_HE_Grenade_shell";
		};
		class RL_Glock_17 : base_list_weapon {};
		class hlc_rifle_awmagnum_BL : base_list_weapon {
			kills = 3;
			scope = "optic_dms";
		};

	};

	class vanilla { //33 kills total

		class srifle_DMR_06_olive_F : base_list_weapon {
			scope = "hlc_optic_docterr";
			mag = "hlc_20Rnd_762x51_B_M14";
			mag_count = 6;
		};

		class arifle_Katiba_F : base_list_weapon {};

		class arifle_Mk20_plain_F : base_list_weapon {};

		class arifle_MX_SW_Black_F : base_list_weapon {
			mag = "100Rnd_65x39_caseless_black_mag_tracer";
		};

		class SMG_03_TR_black : base_list_weapon {};

		class srifle_DMR_01_F : base_list_weapon {};

		class srifle_DMR_02_F : base_list_weapon {
			scope = "optic_ams";
			mag = "ACE_20Rnd_762x67_Berger_Hybrid_OTM_Mag";
		};

		class srifle_DMR_03_F : base_list_weapon {
			scope = "optic_erco_blk_f";
		};

		class LMG_03_F : base_list_weapon {
			scope = "optic_hamr";
		};

		class arifle_SPAR_01_snd_F : base_list_weapon {
			scope = "optic_hamr";
		};

		class arifle_CTAR_blk_F : base_list_weapon {
			scope = "ace_optic_hamr_2d";
		};

		class arifle_ARX_blk_F : base_list_weapon {
			mag = "10Rnd_50BW_Mag_F";
			scope = "ace_optic_hamr_2d";
		};

		class arifle_MSBS65_black_F : base_list_weapon {};

		class sgun_HunterShotgun_01_F : base_list_weapon {};

		class arifle_RPK12_F : base_list_weapon {};

		class LMG_Mk200_black_F : base_list_weapon {};

		class arifle_SPAR_03_blk_F : base_list_weapon {};

		class MMG_02_camo_F : base_list_weapon {};

		class srifle_GM6_F : base_list_weapon {
			kills = 3;
		};

	};

	class update2 { //27 kills total

		class MMG_02_black_F : base_list_weapon {};

		class srifle_EBR_F : base_list_weapon {};

		class hlc_rifle_falosw : base_list_weapon {};

		class hlc_rifle_416D20_wdl : base_list_weapon {};

		class hlc_rifle_PSG1A1_RIS : base_list_weapon {
				scope = "optic_dms";
		};

		class hlc_rifle_awMagnum_BL_ghillie : base_list_weapon {
				scope = "optic_dms";
		};

		class hlc_rifle_awcovert_FDE : base_list_weapon {
				scope = "optic_dms";
		};

		class RL_MP7_Pistol_Black_20Rnd : base_list_weapon {
				scope = "ace_optic_lrps_2d";
		};

		class hlc_Pistol_M11 : base_list_weapon {};

		class hgun_Pistol_01_F : base_list_weapon {};

		class RL_SilverBaller : base_list_weapon {};

		class RL_Glock_17 : base_list_weapon {};

		class RL_crossbow_wood : base_list_weapon {};

	};

	/*
		class russian {  //24 kills total
			class hlc_rifle_slr107u : base_list_weapon {};

			class hlc_rifle_ak47 : base_list_weapon {};

			class hlc_rifle_ak74 : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_ak74_dirty2 : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_ak74m : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_akm : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_aks74 : base_list_weapon {};

			class hlc_rifle_aks74u : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_rpk : base_list_weapon {};

			class hlc_rifle_RK62 : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_aek971 : base_list_weapon {
				kills = 2;
			};

			class hlc_rifle_aks74_GL : base_list_weapon {
				mag_count = 3;
				mag = "hlc_VOG25_AK";
			};
			class hlc_rifle_FN3011Modern : base_list_weapon {
				kills = 2;
				mag_count = 3;
				scope = "hlc_optic_vomz";
			};
		};
	*/
/*
	class update3 { //kills 30

		class arifle_MXM_Black_F : base_list_weapon {
				scope = "optic_ERCO_blk_F";
				mag = "30Rnd_65x39_caseless_black_mag";
				muzzles = "muzzle_snds_65_TI_blk_F";
				kills = 3;
		};

		class arifle_AK12_F : base_list_weapon {
			kills = 3;
		};

		class arifle_SPAR_03_blk_F : base_list_weapon {
			kills = 3;
		};

		class SMG_05_F : base_list_weapon {
			kills = 3;
		};

		class SMG_03C_black : base_list_weapon {};

		class hlc_rifle_honeybadger : base_list_weapon {
				scope = "optic_ERCO_blk_F";
				mag = "29rnd_300BLK_STANAG";
				muzzles = "muzzle_HBADGER";
				kills = 3;
		};

		class hlc_rifle_FAL5000 : base_list_weapon {
			scope = "";
			kills = 3;
		};

		class hlc_rifle_ACR_MID_black : base_list_weapon {
				scope = "optic_Yorris";
				mag = "hlc_30rnd_556x45_EPR";
				muzzles = "hlc_muzzle_556NATO_M42000";
				kills = 3;
		};

		class hlc_rifle_auga2carb_b : base_list_weapon {
			kills = 3;
		};

		class hlc_pistol_DL44 : base_list_weapon {
			scope = ""
			kills = 3;
		};
	};

	class assaultrifles {  //57 kills total
		class hgun_PDW2000_F : base_list_weapon {
			kills = 1;
		};
		class hlc_rifle_M4 : base_list_weapon {};
		class hlc_rifle_M4a1carryhandle : base_list_weapon {};
		class hlc_rifle_CQBR : base_list_weapon {
		};
		class hlc_rifle_m4m203 : base_list_weapon {
			mag = "1Rnd_HE_Grenade_shell";
			kills = 1;
		};
		class hlc_rifle_Colt727 : base_list_weapon {};
		class hlc_wp_mod727 : base_list_weapon {};
		class hlc_wp_mod733 : base_list_weapon {
		};
		class hlc_rifle_vendimus : base_list_weapon {
			kills = 2;
		};
		class hlc_rifle_G36C : base_list_weapon {};
		class hlc_rifle_G36E1 : base_list_weapon {};
		class hlc_rifle_G36KA1KSK : base_list_weapon {};
		class hlc_rifle_g3a3ris : base_list_weapon {};
		class hlc_rifle_G36KV : base_list_weapon {};
		class hlc_rifle_G36V : base_list_weapon {
		};
		class hlc_rifle_RU5562 : base_list_weapon {};
			kills = 3;
		class hlc_rifle_RU556 : base_list_weapon {
			kills = 3;
		};
		class hlc_rifle_416D10 : base_list_weapon {};
		class hlc_rifle_416D20 : base_list_weapon {

		};
		class hlc_rifle_ACR68_full_black : base_list_weapon {};

		class srifle_GM6_F : base_list_weapon {
			kills = 3;
			scope = "optic_dms";
		};

	};

    class update { //35 kills total (waffen aus neuem update)

		class arifle_RPK12_F : base_list_weapon {
			kills = 2;
			scope = "optic_mrco";
		};

		class arifle_MSBS65_sand_F : base_list_weapon {
			kills = 3;
			scope = "optic_mrco";
		};

		class srifle_DMR_06_hunter_F : base_list_weapon {
			kills = 3;
			scope = "optic_hamr";
		};

		class sgun_HunterShotgun_01_F : base_list_weapon {
			kills = 3;
		};

		class arifle_AK12_lush_F : base_list_weapon {
			kills = 2;
			scope = "optic_mrco";
		};

		class hgun_Pistol_heavy_01_green_F : base_list_weapon {
			kills = 2;
			scope = "optic_mrd_black";
		};

		class RL_SilverBaller : base_list_weapon {
			kills = 2;
		};

		class RL_MP7_Pistol_Black_20Rnd : base_list_weapon {
			kills = 3;
			scope = "optic_lrps";
		};

		class RL_Desert_Eagle : base_list_weapon {
			kills = 4;
		};

		class RL_Glock_17 : base_list_weapon {
			kills = 2;
		};

		class sgun_HunterShotgun_01_sawedoff_F : base_list_weapon {
			kills = 2;
		};

		class RL_crossbow_black : base_list_weapon {
			kills = 2;
			scope = "optic_aco_smg";
		};

	};*/
};

/*
			 random
				{"hlc_rifle_saiga12k",                  3, "<auto>", 6},
				{"hgun_Pistol_heavy_02_F", 1, "<auto>", 5},
				{"hgun_Pistol_01_F", 1, "<auto>", 5},
				{"hgun_Rook40_F", 1, "<auto>", 5},
				{"hlc_smg_mp5k", 1, "<auto>", 9},
				{"hlc_smg_mp5a2", 1, "<auto>", 5},
				{"hlc_smg_mp5sd6", 2, "<auto>", 5},
				{"arifle_AKS_F", 1, "<auto>", 5},
				{"arifle_AK12_F", 1, "<auto>", 5},
				{"arifle_AKM_F",        1, "<auto>", 5},
				{"hlc_rifle_RPK12",         1, "<auto>", 5},
				{"hlc_rifle_bab",               2, "<auto>", 5},
				{"hlc_rifle_bcmjack",       1, "<auto>", 5},
				{"hlc_rifle_RU5562",            2, "<auto>", 5},
				{"hlc_rifle_vendimus",      2, "<auto>", 5},
				{"hlc_rifle_SAMR",              1, "<auto>", 5},
				{"launch_RPG7_F", 1, "RPG7_F", 4},
				{"hlc_rifleACR_SBR_cliffhanger",                2, "<auto>", 5},
				{"arifle_CTARS_ghex_F",                 1, "<auto>", 5},
				{"SMG_03C_camo",        1, "<auto>", 5},
				{"MMG_02_sand_F", 1, "<auto>", 5},
				{"hlc_rifle_ak47",              2, "<auto>", 5},
				{"hlc_rifle_ak74_MTK",      1, "<auto>", 5},
				{"arifle_SPAR_03_snd_F",            2, "<auto>", 5},
				{"hlc_rifle_G36E1AG36_Romi",        2, "<auto>", 5},
				{"hlc_rifle_psg1",              1, "<auto>", 5},
				{"hlc_rifle_m14sopmod",             2, "<auto>", 6},
				{"srifle_DMR_03_khaki_F", 2, "<auto>", 5},
				{"hlc_rifle_saiga12k",              2, "<auto>", 5},
				{"LMG_Mk200_F",         2, "<auto>", 5},
				{"hlc_Pistol_M11",          1, "<auto>", 5},
				{"hlc_pistol_P229R_357Elite",       1, "<auto>", 5},
				{"hlc_Pistol_M11A1D",               1, "<auto>", 5},
